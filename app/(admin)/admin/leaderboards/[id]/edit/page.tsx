...
import { ArrowLeft, X, Trash } from "lucide-react"
...
// Function to calculate points based on placement (for Battle Royale)
...
// Function to calculate total points for a map (for Battle Royale)
...
export default function EditTournamentLeaderboardsPage() {
  ...
  const handleMapResultChange = (teamIndex: number, map: string, field: string, value: number) => {
    ...
    if (tournament.format === "Battle Royale") {
      ...
      // For Clash Squad, only update total kills
      team.totalKills = Object.values(team.mapResults).reduce((sum, mapResult) => sum + mapResult.kills, 0)
    }
  ...
  const handleSave = () => {
    ...
    router.push(`/admin/leaderboards/${params.id}/view`)
  }
  ...
  const handleDeleteLeaderboard = async () => {
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to delete the leaderboard. Please try again.",
        variant: "destructive",
      })
    }
  }
    ...
    <AdminLayout>
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold">Edit {tournament.name} Leaderboards</h1>
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This action cannot be undone. This will permanently delete the leaderboard for {tournament.name}.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
                <AlertDialogAction onClick={handleDeleteLeaderboard}>Delete</AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
          </AlertDialog>
        </div>
